...
{{ list_articles length="1" ignore_issue="true" }}
              {{ if $gimme->article->has_image(2) }}<div class="article-image">
                  ...
                  </div><!-- /.image-description -->{{ /if }}
                  
                </div><!-- /.article-image -->{{ /if }}
...
{{ /list_article_images }}      
     ...
     {{ if $image_count }}<img alt="." src="/templates/images/midplus.png" /><a class="something-count" href="{{ uri options="article" }}">{{ $image_count }}</a>{{ /if }}{{ if $gimme->article->comment_count gt 0 }}<a class="comment-count" href="{{ uri options="article" }}">{{ $gimme->article->comment_count }}</a>{{ /if }}</p></div>
