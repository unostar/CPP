...
$script_path = '/templates/slonim/phpscripts/formmail/send.php';
...
$recipients = 'gs@gs.by'; // selrecip;

// HTML PAGE TITLE
// Text to show in html page <title> tag
$html_page_title = 'Газета Слонiмская - Сообщение';
...
$form_struct = '
  type=openfieldset|legend=Отправить сообщение
//  type=selrecip|class=fmselect|label=Послать|data=(выберите),Присылайте свои рассказы,pressa@gs.by,Присылайте фотографии,pressa@gs.by,Напишите нам,pressa@gs.by
	type=text|class=fmtext|label=Ваше имя|fieldname=fm_name|max=100|req=true
	type=text|class=fmtext|label=Ваш email|fieldname=fm_email|max=100|req=true|ver=email
	type=text|class=fmtext|label=Тема|fieldname=fm_subject|max=100|req=true
	type=verify|class=fmverify|label=Проверка
	type=textarea|class=fmtextarea|label=Текст послания|fieldname=fm_message|max=1000|rows=6|req=true
  type=file|class=fmfile|label=Файл|fieldname=fm_upload|allowed=jpg,jpeg,gif,png,pdf,zip,rar,doc,docx,rtf
  type=closefieldset
';
...
$sent_message = '<p>Спасибо, Ваше сообщение успешно отправлено.</p>';
...
//					$o .= $sent_message;
            $o .= $sent_message . '<br /><center><input type="button" value="Закрыть" onclick="javascript:window.close();" /></center>';
