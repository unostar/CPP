...
$rss_file = "http://belapan.com/rss/ru_fresh_news.xml";
...
foreach ($rss_feed->channel->item as $item ) {
	...
	$content .= "<li class=\"news\"><a rel=\"nofollow\" href=\"$item->link\">".trim($item->title)."</a> <small class=\"time\">($time[1] $time[2] $time[3])</small></li>\n";
}

$content = '<ul class="imported-news">'.$content.'</ul>';

if (strlen($content) > 1024) {
	file_put_contents("/var/www/gsby/campsite/templates/slonim/slonim-feed-belapan.tpl", $content);
}
