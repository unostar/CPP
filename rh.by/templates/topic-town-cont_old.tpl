...
{{ /if }}
   ...
   {{ if $gimme->current_list->at_end }}
      ...
      {{ if $gimme->current_list->has_previous_elements }}
         <span><a href="{{uripath options="section"}}?{{urlparameters options="prev_items" }}">Папярэдняя</a></span>
      {{ else }}
         <span>Папярэдняя</span>
      {{ /if }}
      <span>|</span>
      {{ if $gimme->current_list->has_next_elements }}
         <span><a href="{{uripath options="section"}}?{{urlparameters options="next_items" }}">Наступная</a></span>
      {{ else }}
         <span>Наступная</span>
      {{ /if }}
