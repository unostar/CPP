...
$script_path = 'http://www.belmarket.by/templates/phpscripts/formmail/sub-net.php';
...
$form_struct = '
type=select|class=fmselect|label=Вид|fieldname=fm_type|req=true|data=(выберите),баннерная реклама,размещение в разделе "Новости компаний"
type=select|class=fmselect|label=Типы баннеров|fieldname=fm_typea|req=true|data=(выберите),верхний баннер 990х60px,баннер в центре 630х90px,левый баннер 140х250px,нижний баннер 728х90px
	type=text|class=fmtext|label=Период размещения|fieldname=fm_other|max=100|req=true
	type=text|class=fmtext|label=Плательщик|fieldname=fm_other1|max=100|req=true
	type=textarea|class=fmtext|label=Адрес|fieldname=fm_address|max=1000|rows=4|req=true
	type=textarea|class=fmtext|label=УНН|fieldname=fm_unn|max=1000|rows=4|req=true
	type=textarea|class=fmtext|label=ОКПО|fieldname=fm_okpo|max=1000|rows=4|req=true
	type=textarea|class=fmtext|label=Р/С|fieldname=fm_account|max=1000|rows=4|req=true
	type=text|class=fmtext|label=Наименование банка|fieldname=fm_bankname|max=100|req=true
	type=text|class=fmtext|label=Код банка|fieldname=fm_bankcode|max=100|req=true
	type=text|class=fmtext|label=Телефон|fieldname=fm_phone|max=100|req=true
	type=text|class=fmtext|label=Факс|fieldname=fm_fax|max=100|req=false
	type=text|class=fmtext|label=E-mail|fieldname=fm_email|max=100|req=true|ver=email
	type=text|class=fmtext|label=Директор|fieldname=fm_director|max=100|req=true
	type=text|class=fmtext|label=Гл.бухгалтер|fieldname=fm_acountant|max=100|req=true
	type=text|class=fmtext|label=Исполнитель|fieldname=fm_executor|max=100|req=true
type=textarea|class=fmtext|label=Доп.информация|fieldname=fm_extra|max=1000|rows=6|req=false
...
$sender_name = 'fm_director';
$sender_email = 'fm_email';
$email_subject = 'Заказ рекламы для размещения на сайте';
...
");
	...
	if (!$message_sent) {	
		...
		if (trim($manual_form_code) == '') {	// ** Use normal form generation
			...
			$o .= '<form class="ddfm" id="subscribe" method="post" action="' . $path_contact_page . '" enctype="multipart/form-data">' . "
...
";
			...
			$o .= '<div class="credits">' . DDFM_CREDITS . ' </div>' . "
