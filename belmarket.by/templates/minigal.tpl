...
<div class="contentdiv"><a href="{{uri options="image"}}" rel="minigalZoom" title="{{$gimme->image->description}} // Фото: {{$gimme->image->photographer}}" style="background:url({{uri options="image"}}&ImageRatio=38) no-repeat center;"></a>{{$gimme->image->description}}</div>
{{/if}}
{{ /list_article_images }}
</div>
...
{{list_article_images}}
{{if $gimme->image->article_index > 99}}
<a href="#" class="toc" style="background:url({{uri options="image"}}&ImageRatio=8) no-repeat center;" title="{{$gimme->image->description}}"></a>
{{/if}}
{{ /list_article_images }}
</div>
