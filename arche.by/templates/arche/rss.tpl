...
<image>
<url>http://{{$gimme->publication->site}}/templates/arche/img/rss-logo.png</url>
<title>{{$gimme->publication->name}}{{if $gimme->section->defined && $gimme->section->url_name != "rss"}} - {{$gimme->section->name}}{{/if}}</title>
<link>http://{{$gimme->publication->site}}{{if $gimme->section->defined && $gimme->section->url_name != "rss"}}/{{$gimme->language->code}}/page/{{$gimme->section->url_name}}/{{/if}}</link>
<width>144</width>
<height>36</height>
...
{{if $gimme->section->defined && $gimme->section->url_name != "rss"}}{{assign var="const" value="section is `$gimme->section->number` type is article"}}{{else}}{{assign var="const" value="section greater 20 section smaller 80 type is article"}}{{/if}}
...
{{* list_article_images}}{{if $gimme->article->image->article_index > 2 }}<media:content url="{{$gimme->image->imageurl}}" medium="image" />{{/if}}{{/list_article_images *}}
