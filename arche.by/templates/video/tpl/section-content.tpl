...
{{assign var="articles_per_page" value="32"}}

{{list_articles columns=$articles_per_page constraints="type is video" ignore_issue="true" order=$sort}}
...
{{if $gimme->current_list->column == 1}}
  ...
  {{else}}
    <li><a href="/{{$gimme->language->code}}/channel/{{$gimme->section->url_name}}/?page={{$gimme->current_list->row}}{{if $smarty.get.sort}}&amp;sort={{$smarty.get.sort}}{{/if}}">{{$gimme->current_list->row}}</a></li>
  {{/if}}
{{/if}}
{{if $gimme->current_list->at_end}}
	</ol>
