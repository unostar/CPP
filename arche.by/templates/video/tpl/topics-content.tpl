...
{{assign var="topic_const" value=$smarty.get.topic|replace:' ':'\ '}}
{{assign var="topic_const" value="topic is `$topic_const`:by"}}
{{assign var="articles_per_page" value="60"}}

{{list_articles columns=$articles_per_page constraints="type is video `$topic_const`" ignore_issue="true" ignore_section="true" order=$sort}}
{{if $gimme->current_list->at_beginning}}<ul class="videos">{{/if}}
...
{{/if}}
{{if $gimme->current_list->at_end}}</ul><!-- .videos -->{{/if}}
{{/list_articles}}
...
{{if $gimme->current_list->column == 1}}
  ...
  {{else}}
    <li><a href="/{{$gimme->language->code}}/channel/topics/?page={{$gimme->current_list->row}}&amp;topic={{$smarty.get.topic|escape:'url':'utf-8'}}{{if $smarty.get.sort}}&amp;sort={{$smarty.get.sort}}{{/if}}">{{$gimme->current_list->row}}</a></li>
  {{/if}}
{{/if}}
