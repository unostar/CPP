...
.send_letter .attachment {display: block; float: left; margin: -1px 5px 0 3px; width: 24px; height: 24px; background:url(/templates/video/img/attachment.png) no-repeat;}
...
require_once('/var/www/arche/campsite/templates/video/php/recaptchalib.php');
...
if ($name && $email && $text && $recaptcha){
  $to = "blackpig@tut.by";
  $from = "Arche.by <noreply@video.arche.by>";
  $subject = 'Паведамленне з сайта video.arche.by'.$topic;
  $message = "З сайта video.arche.by была адпраўлена паведамленне.
