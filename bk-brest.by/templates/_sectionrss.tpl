{{set_current_issue}}{{set_section number=$smarty.get.section}}<rss version="2.0" xmlns:atom="http://www.w3.org/2005/Atom" xmlns:media="http://search.yahoo.com/mrss/">
<channel>
...
<atom:link href="http://{{$gimme->publication->site}}/{{$smarty.get.section_name}}.rss" rel="self" type="application/rss+xml" />
{{list_articles length="20" constraints="type is article section is `$smarty.get.section`" ignore_issue="true" order="bypublishdate desc"}}
<item>
<title>{{$gimme->article->name|html_entity_decode|regex_replace:'/&(.*?)quo;/':'&quot;'}}</title>
<link>{{url options="article"}}</link>
<description>
{{if $gimme->article->has_image(1)}}
&lt;img src="{{$gimme->article->image1->imageurl}}" width="100" height="100" border="0" align="left" hspace="5" /&gt;
{{/if}}
