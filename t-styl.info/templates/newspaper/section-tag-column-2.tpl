...
{{ set_default_section }}
	...
	{{ list_articles length="1" constraints="topic is $tag:by" }}
...
{{ /local }}
	...
	{{ list_articles columns="7" ignore_issue="true" length="35" constraints="topic is $tag:by" }}
		{{if $gimme->current_list->row == $page || $page > 5 }}

			{{ if $gimme->current_list->at_beginning && $gimme->current_list->count > 1 }}
				<h2 class="other">Iншыя Артыкулы з тэгам <span style="font-size:9px;">&lt;&lt;</span> {{ $tag }} <span style="font-size:9px;">&gt;&gt;</span></h2>
			{{ /if }}
...
{{ list_articles columns="7" ignore_issue="true" length="35" constraints="topic is $tag:by" }}
	{{if $gimme->current_list->count > 7}}
		...
		{{if $gimme->current_list->column == 1}}
			...
			{{else}}
				<li><a href="/?tpl=582&tag={{ $tag }}&page={{$gimme->current_list->row}}">{{$gimme->current_list->row}}</a></li>
			{{/if}}
