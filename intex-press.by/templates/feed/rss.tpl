...
<channel>
<title>{{$siteinfo.title}}{{if $smarty.get.section}} - {{$gimme->section->name}}{{/if}}</title>
...
<image>
<url>http://{{$gimme->publication->site}}/templates/images/rss_logo.gif</url>
<title>{{$siteinfo.title}}{{if $smarty.get.section}} - {{$gimme->section->name}}{{/if}}</title>
<link>http://{{$gimme->publication->site}}{{if $smarty.get.section}}{{uri options="section"}}{{/if}}</link>
<width>100</width>
<height>35</height>
...
{{if $smarty.get.section}}{{assign var="const" value="section is `$smarty.get.section`"}}{{else}}{{assign var="const" value="section smaller 100 section not 5 type is article"}}{{/if}}
